    ...
    function render() {
        ...
        if (localPlayers.size === 0) {
            ...
            localPlayers.forEach((p, sid) => {
                ...
                const c = p.characterId && p.characterId in CHARACTERS ? CHARACTERS[p.characterId as CharacterId] : null;
                ...
                ready.textContent = p.ready ? "READY" : "being an idiot";
    ...
    function bindPlayer(player: LobbyPlayerSchema, sessionId: string) {
        ...
        player.onChange(() => {
            ...
            if (sessionId === room.sessionId) {
                myPick = player.characterId && player.characterId in CHARACTERS ? (player.characterId as CharacterId) : null;
